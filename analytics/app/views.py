...
def home(request):
    ...
    url_labels = []
    url_values = []
    requests_by_url = Request.objects.filter(url__isnull=False) .values("url").annotate(total_unique_visitors = Count("ip_address", unique=True)).filter(total_unique_visitors__gt=50).order_by("-total_unique_visitors")
